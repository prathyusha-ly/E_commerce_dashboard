# E_commerce_dashboard
Developed an interactive Sales Dashboard in Excel to analyze monthly and category-wise performance. Used charts, Pivot Tables, and Combo Box to visualize sales and profit trends. Enabled users to explore patterns, identify top-performing categories, and support data-driven decision-making.
# 🔹 Overview
An Excel dashboard designed to analyze e-commerce sales performance across regions, products, and time periods. It presents key metrics and trends using structured data and visualizations to support business insights.
# 🔹Dashboard
<img width="854" height="278" alt="Sales Dashboard-combo box" src="https://github.com/user-attachments/assets/7640087f-19fd-4d17-a8a0-6b524855bf92" />

# 🔹Key Insights
- Sales show a steady increase from the beginning of the year, reaching peak performance around October.
- The Central region contributes the highest sales compared to other regions.
- Profit trends generally follow sales, with some fluctuations across months.

## 📂 Project File
Download the Excel file from this repository to explore the dashboard and analysis.

<a href= "https://github.com/prathyusha-ly/E_commerce_dashboard/blob/main/E_Commerce_Dashboard.xlsx">Data set</a>



